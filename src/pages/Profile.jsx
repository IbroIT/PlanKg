    ...
    <div className="min-h-screen bg-[#E9EEF4] py-4 md:py-8">
      <div className="container mx-auto px-3 md:px-4">
        <div className="max-w-6xl mx-auto">
          {/* Header Section */}
          <div className="bg-linear-to-r from-[#1E2A3A] to-[#2a3f54] rounded-2xl shadow-2xl p-4 md:p-8 mb-6 md:mb-8 text-white">
            <div className="flex flex-col md:flex-row items-center justify-between">
              <div className="flex items-center mb-4 md:mb-0">
                <div className="relative">
                  <div className="w-16 h-16 md:w-24 md:h-24 bg-linear-to-br from-[#F4B942] to-[#e5a832] rounded-full flex items-center justify-center shadow-2xl">
                    {user?.avatar ? (
                      <img
                        src={user.avatar}
                  ...
                  <div className="absolute bottom-1 right-1 md:bottom-2 md:right-2 w-4 h-4 md:w-6 md:h-6 bg-green-500 border-2 md:border-4 border-white rounded-full"></div>
                </div>
                <div className="ml-3 md:ml-6">
                  ...
                  {user?.role === 'provider' && (
                    <p className="text-gray-300 text-sm md:text-base">{t('auth.provider')}</p>
                  )}
                </div>
              </div>
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 w-full md:w-auto">
                {!editMode && (
                  <button
                    ...
                    className="bg-[#F4B942] text-[#1E2A3A] px-4 py-2 md:px-6 md:py-3 rounded-xl font-bold hover:bg-[#e5a832] transition-all duration-300 shadow-lg hover:shadow-xl text-sm md:text-base"
                  >
                ...
                <button
                  ...
                  className="bg-white text-[#1E2A3A] px-4 py-2 md:px-6 md:py-3 rounded-xl font-bold hover:bg-gray-100 transition-all duration-300 shadow-lg hover:shadow-xl text-sm md:text-base"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            ...
            <div className="md:col-span-1">
              ...
              <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 mb-4 md:mb-6">
                <h2 className="text-lg md:text-xl font-bold text-[#1E2A3A] mb-3 md:mb-4">{t('profile.profileInfo')}</h2>
                
                {editMode ? (
                  <form onSubmit={handleUpdate} className="space-y-3 md:space-y-4">
                    <div>
                      <label className="block text-sm font-semibold text-[#1E2A3A] mb-1 md:mb-2">
                        {t('auth.firstName')}
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows="3 md:rows-4"
                        className="w-full px-3 py-2 md:px-4 md:py-3 bg-[#E9EEF4] border-0 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#F4B942] text-[#1E2A3A] text-sm md:text-base resize-none"
                      ></textarea>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-[#1E2A3A] mb-1 md:mb-2">
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                        <div className="relative shrink-0">
                          <div className="w-16 h-16 md:w-20 md:h-20 bg-[#E9EEF4] border-2 border-dashed border-[#F4B942] rounded-full flex items-center justify-center overflow-hidden">
                            {avatarPreview ? (
                              <img
                                src={avatarPreview}
                                alt="Avatar preview"
                              ...
                              <svg className="w-6 h-6 md:w-8 md:h-8 text-[#F4B942]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 pt-3 md:pt-4">
                      <button
                        type="submit"
                        className="flex-1 bg-[#F4B942] text-[#1E2A3A] px-4 py-2 md:px-6 md:py-3 rounded-xl font-bold hover:bg-[#e5a832] transition-all duration-300 shadow-lg hover:shadow-xl text-sm md:text-base"
                      >
                        {t('profile.save')}
                      </button>
                      <button
                        ...
                        className="flex-1 bg-gray-200 text-[#1E2A3A] px-4 py-2 md:px-6 md:py-3 rounded-xl font-bold hover:bg-gray-300 transition-all duration-300 text-sm md:text-base"
                      >
                        {t('addService.cancel')}
                      </button>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2">
                      ...
                      <div className="space-y-2">
                        {user?.whatsapp ? (
                          <div className="flex items-center space-x-2 text-[#1E2A3A] bg-[#E9EEF4] p-2 md:p-3 rounded-xl">
                          ...
                          <div className="flex items-center space-x-2 text-gray-400 bg-gray-100 p-2 md:p-3 rounded-xl">
                ...
                {(!user?.city && !user?.phone && !user?.whatsapp && !user?.telegram && !user?.instagram) && (
                  <div className="mt-4 md:mt-6 bg-[#E9EEF4] rounded-xl p-3 md:p-4 border-l-4 border-[#F4B942]">
                    <div className="flex items-start space-x-2 md:space-x-3">
                      <svg className="w-5 h-5 md:w-6 md:h-6 text-[#F4B942] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                      </svg>
                      <div>
                        <h4 className="font-semibold text-[#1E2A3A] mb-1 text-sm md:text-base">{t('profile.fillProfile', 'Заполните профиль')}</h4>
                        <p className="text-gray-600 text-xs md:text-sm">{t('profile.fillProfileDesc', 'Добавьте город и контактные данные, чтобы клиенты могли легко связаться с вами')}</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              {services.length > 0 && (
                <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6">
                  <h3 className="text-base md:text-lg font-bold text-[#1E2A3A] mb-3 md:mb-4">{t('profile.stats')}</h3>
            ...
            <div className="md:col-span-2">
              ...
              <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 md:mb-6">
                  <h2 className="text-xl md:text-2xl font-bold text-[#1E2A3A] mb-3 sm:mb-0">{t('profile.myServices')}</h2>
                  
                  {/* Status Tabs */}
                  {services.length > 0 && (
                    <div className="flex flex-wrap gap-1 md:gap-2 bg-[#E9EEF4] p-1 rounded-xl">
                      <button
                          onClick={() => setActiveTab('all')}
                          className={`px-2 py-1 md:px-4 md:py-2 rounded-lg font-semibold transition-all text-xs md:text-sm ${
                            activeTab === 'all' 
                ...
                {services.length > 0 ? (
                  ...
                  <div className="text-center py-4 md:py-12">
                    <div className="w-16 h-16 md:w-24 md:h-24 mx-auto mb-3 md:mb-4 bg-[#E9EEF4] rounded-full flex items-center justify-center">
                    ...
                    <button
                      onClick={() => navigate('/add-service')}
                      className="bg-[#F4B942] text-[#1E2A3A] px-4 py-2 md:px-8 md:py-3 rounded-xl font-bold hover:bg-[#e5a832] transition-all duration-300 shadow-lg hover:shadow-xl text-sm md:text-base"
                    >
                      {t('profile.addService')}
                    </button>
                  </div>
